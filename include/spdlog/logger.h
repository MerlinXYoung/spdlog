...
#error SPDLOG_WCHAR_TO_UTF8_SUPPORT only supported on windows
    ...
    inline void set_level(level::level_enum log_level) SPDLOG_NOEXCEPT
    {
        level_.store(log_level);
    }

    inline level::level_enum level() const SPDLOG_NOEXCEPT
    {
        return static_cast<level::level_enum>(level_.load(std::memory_order_relaxed));
    }

    inline const std::string &name() const SPDLOG_NOEXCEPT
    {
        return name_;
    }
